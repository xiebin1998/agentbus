...
| TASK-19 | Daemon 指标上报【✅ 已完成 2026-08-09：286 TS + 69 Python 全绿；daemon 侧 MetricsCollector + 周期 publish 到 /agenthub/ai/metric/<ns>/<cid>（router drop 新增 kind 分类：invalid/whitelist/dedup/hop）；hub 侧独立 MQTT 连接订阅 metric/# 汇总 + /health 扩展 daemon_metrics；真机端到端冒烟通过（injected_ok/deduped/dropped 计数 + senders + report_count 周期上报均正确），分支 feat/task-19】 | T15 | hub 可查各 daemon 指标 |
...
| TASK-22 | 二期集成验收【✅ 已完成 2026-08-09：289 TS + 88 Python 全绿（tsc 无错）；架构 10 章二期清单 9 项逐项核对（8 项完全达成）；六工具矩阵回归：真机 init（六工具）→doctor→status→uninstall 零残留全链路 + daemon 指标上报闭环；回归发现并 TDD 修复三缺陷：①.cmd 参数含 & 被 cmd.exe 分割致 codex 注册恒失败（escapeCmdArg+windowsVerbatimArguments）②doctor 探测 localhost→::1 脑裂（回退 127.0.0.1）③cli 型注册幂等（remove-then-add）；验收报告 docs/acceptance-phase2.md，★ 二期里程碑达成，分支 feat/task-22】 | T18 | 六工具矩阵回归 |
...
| TASK-25 | 安全基线（鉴权/TLS/SSE）【✅ 已完成 2026-08-10：106 Python + 304 TS 全绿；hub token 鉴权（MCP_API_TOKEN 非空启用，纯 ASGI 中间件保护 /sse、/messages/*、/console、/api/console/*，/health 开放，query/Bearer 双通道）；broker 安全基线：mosquitto allow_anonymous false + password_file + TLS 8883（scripts/setup-broker-security.ps1 生成自签 CA/服务端证书与密码）；daemon TLS CA（buildConnectOptions 纯函数 + broker.ca 支持 ${ENV}）；hub MQTT_CA_CERTS 信任锚；真机冒烟：匿名被拒/错密码被拒/密码认证/TLS 端到端（TLSv1.3 + 自签 CA + daemon mqtts 订阅）/SSE 401→200 全通过；metric 端到端测试适配认证（无凭证 skip）；分支 feat/task-25】 | T21 | 匿名被拒、TLS 端到端 |
| TASK-26 | 账号/团队管理【✅ 已完成 2026-08-10：112 Python + 33 TS 文件全绿（tsc=0）；团队登记 TeamStore + 控制台 API（GET/POST /api/console/teams、DELETE /{name}，受 TASK-25 token 鉴权保护）；一团队一账号：broker 账号 team-<名> + topic 前缀 ACL（render_broker_acl 纯函数，channel readwrite + metric write）；scripts/sync-broker-acl.ps1 从 hub 拉团队清单 → 建账号 → 渲染 ACL → SIGHUP 热加载（幂等）；setup 脚本同步生成初始 ACL；真机验收：跨团队 publish 被拒（broker 日志 Denied PUBLISH + 订阅方未收到，mosquitto 2.1 静默丢弃语义已实测）、团队内送达、hub 全量账号不受影响；待补：控制台前端团队页、SSE 接入身份与团队归属校验；分支 feat/task-26】 | T22 | 跨团队 publish 被 ACL 拒 |
| TASK-27 | 进阶通道（serve/SDK）【✅ 已完成 2026-08-10：343 TS + 112 Python 全绿（tsc=0）；opencode serve 无头长活进程 + run --attach 免冷启动注入：ServeManager（binary+workspace+port 缓存/双流解析服务地址/探测重拉/启动失败回退冷启动）+ 适配器 attach 回合（part.text/sessionID 实测形态）+ base.ts resolveSpawnTarget 复用；真机实测：attach 回合 CLI 直测 1133ms vs 冷启动 16059ms（约 14 倍），模型生成仅 0~36ms，daemon 端到端第 2 回合 3.5~5.4s（本地模型 9900+ tokens 预处理主导，通道开销约 1.1s）；回退机制两次实测生效；连环缺陷 TDD 修复：stderr 地址解析/taskkill /T 后固定端口无条件复查补杀/netstat 受限回退 PowerShell Get-NetTCPConnection/Windows SIGTERM 实测为强杀致优雅回收不运行→daemon stop 与 uninstall 按 serve_port 定向回收孤儿 serve（stop 后端口零残留实测通过）；待补：kilo serve（7.4.17 无 serve 子命令，待工具支持）、Linux 下 serve 回收路径；分支 feat/task-27】 | T23 | 注入亚秒级 |
| TASK-28 | 一键安装脚本【✅ 已完成 2026-08-10：349 TS + 115 Python 全绿（tsc=0）；scripts/install.ps1 + install.sh（架构 6.6 三步流程：Node>=18 环境门槛 → npm 全局装 CLI（AGENTBUS_PACKAGE 可指本地包/离线 tarball）→ init --yes → doctor，每步失败即停带网络/离线/无 CLI 提示）；hub 静态托管 GET /install.ps1 与 /install.sh（引导资源，与 /health 同级鉴权豁免，3 例 Python 测试含防漂移比对）；补契约缺口：init --yes 未指定工具时自动探测全部已知 CLI 取已安装集（2 例）；真机冒烟：干净目录跑 install.ps1 全链路 EXIT_CODE=0（npm 全局装成功 → 自动探测 5 CLI → init 五步全过 → doctor 体检通过）+ uninstall --yes 零残留闭环；待补：干净机器真验（本次为同机干净目录）、install.sh 的 Linux/macOS 实跑（本机无环境）；分支 feat/task-28】 | T24 | 干净机器一条命令接入 |
| TASK-29 | hermes 端到端联调【✅ 已完成 2026-08-10：357 TS + 116 Python 全绿（tsc=0）；并发不串话：SessionLock（Promise 链尾锚点模式，同键 FIFO 串行/异键并行/前序错不阻塞/链尾清键防泄漏）+ daemon 按 工具｜发件人 串行接线（限速队列续排同入口），集成测试断言同源慢回合 maxActive=1 + 严格到达顺序（RED maxActive=3）；重连自愈：listener reconnectPeriod 2s + clean:false，aedes 关闭重建同端口端到端测试 GREEN（先 broker.close 再 server.close，否则 TCP close 互等挂死），offline→自动重连→重订阅→消息送达全链路；出站身份独立（架构 5.5-B）：TASK-24 共享连接模型下 close() 仅移除自身会话，验收测试固定不互踢契约；真机冒烟：docker restart broker 后 daemon 2s 自动重连，4 条注入计数全对；连带挖出并 TDD 修复真实缺陷：qoder/claude runTurn 的 else 分支在 spawn 失败时把 error 覆写成 undefined（daemon 静默判“成功”），改 exitCode===0 且无既有 error 才做逻辑检查（回归用例×2 RED→GREEN）；待补：24h 长跑、hermes 真机并发与 SSH 中断专项（无 hermes 服务器）；分支 feat/task-29】 | T25 | 并发不串话、重连自愈 |
| TASK-30 | OS 级隔离（可选）【✅ 已完成 2026-08-10：377 TS + 116 Python 全绿（tsc=0，41 文件）；架构 4.7 三层防线之隔离层：config isolation（可选默认关）→ daemon 在 readonly 回合窗口内对工作目录施加 OS 层只读（workspace 引用计数，并发异源回合共享隔离，回合全结束才解除；full 回合不施加），回合后自动解除；Windows：icacls deny 当前用户 + .agentbus 排除（daemon 数据目录必须可写）；Linux：chmod -R a-w + .agentbus u+w；doctor 隔离检查项（启用时探测 icacls/chmod 工具链）；CLI isolate apply/remove/status 三子命令（幂等）；真机权限矩阵实测勘误两个 NTFS 陷阱：①继承 ACE 无法 /remove:d，.agentbus 排除必须先 /inheritance:d 转显式再移除；②deny 掩码不可含 D（DELETE）——deny D 会把纯读连 native type 都拒（真机矩阵复现），亦不可用 W（含 WA，libuv 只读打开请求该位）——最终 (OI)(CI)(WD,AD,DC)：读正常、新建/覆写/重命名物理拒（重命名因需源文件删除权限被 DC 拒），副作用：既有文件的纯删除不被拦截；daemon.stop() 改异步（await MQTT 关闭，修复异步 offline 日志 vs 测试删目录的 ENOENT unhandled rejection 竞态，存量测试同步 await 化）；真机冒烟 SMOKE PASS：恶意探针 CLI（.cmd）无视全部 readonly 参数直接写 workspace 根与子目录→均被 OS 物理拒绝（无探针文件 + OS-DENIED）、.agentbus 排除目录可写回合标记、回合后隔离解除（status + 可写验证）；待补：Linux 低权限账号运行方案（本卡以 chmod 机制替代，进阶方案需独立环境）、跨机；分支 feat/task-30】 | T26 | OS 层物理禁写 |
...
- v2.1（2026-08-10）：TASK-25 完成标记；安全基线：hub SSE/控制台 token 鉴权（纯 ASGI 中间件）+ broker 匿名拒绝/密码认证/TLS 8883（setup 脚本生成自签证书）+ daemon broker.ca 信任锚 + hub MQTT_CA_CERTS；真机冒烟匿名/错密码被拒、TLSv1.3 端到端、SSE 401/200 全通过
- v2.2（2026-08-10）：TASK-26 完成标记；账号/团队管理：团队登记 + 控制台 API + 一团队一账号 topic 前缀 ACL（sync-broker-acl.ps1 同步 + SIGHUP 热加载）；真机验收跨团队 publish 被拒（Denied PUBLISH）、团队内可达、hub 不受影响
- v2.3（2026-08-10）：TASK-27 完成标记；进阶通道（serve/SDK）：opencode serve 长活 + run --attach 免冷启动（ServeManager + 回退冷启动）；真机实测 attach CLI 直测 1133ms vs 冷启动 16059ms、模型生成 0~36ms、端到端热路径 3.5~5.4s（模型侧主导）；连环缺陷 TDD 修复（stderr 地址/taskkill 后端口复查/netstat 受限回退 PowerShell/Windows SIGTERM 强杀→stop 与 uninstall 按端口定向回收）；stop 后端口零残留实测通过
- v2.4（2026-08-10）：TASK-28 完成标记；一键安装脚本：install.ps1/install.sh（Node>=18 门槛 + npm 全局装 + init --yes + doctor，错误/离线提示）+ hub 静态托管（鉴权豁免）+ init --yes 工具自动探测补契约缺口；真机冒烟干净目录全链路 EXIT_CODE=0 + uninstall 零残留闭环
- v2.5（2026-08-10）：TASK-29 完成标记；hermes 端到端联调：SessionLock 会话级串行锁（并发不串话，锚点模式）+ daemon runLocked 接线、broker 抖动重连自愈端到端测试（close 顺序陷阱修复）、共享连接下出站身份独立不互踢回归固定；真机冒烟 broker docker restart 后 2s 自愈且注入计数全对；冒烟排障挖出并 TDD 修复真实缺陷：qoder/claude 适配器 spawn 失败 error 被 detectLogicalError 覆写 → daemon 静默判“成功”（else→else if exitCode===0 且无既有 error）
- v2.6（2026-08-10）：TASK-30 完成标记（★ 30/30 收官）；OS 级隔离（可选）：isolation 配置 + daemon readonly 回合窗口 OS 层物理禁写（引用计数并发共享 + .agentbus 排除 + 回合后自动解除）+ doctor 检查 + isolate CLI 三子命令；真机权限矩阵勘误两个 NTFS 陷阱：继承 ACE 需先断继承才能移除；deny 掩码含 D/W 均会把纯读拒 → 最终 (OI)(CI)(WD,AD,DC)（读正常、新建/覆写/重命名物理拒，纯删除不拦为已知边界）；连带 TDD 修复 daemon.stop() 异步竞态（异步 offline 日志 vs 删目录 ENOENT）；真机冒烟恶意探针绕过全部参数层直写被 OS 物理拒绝，SMOKE PASS
