 ...
 * `/phnix/ai/metric/<ns>/<client_id>`，hub（server.py）订阅 `/phnix/ai/metric/#` 汇总可查。
...
describe("metricTopic", () => {
  it("ns 形态：/phnix/ai/metric/<ns>/<client_id>", () => {
    expect(metricTopic("iot", "fe-zhangsan")).toBe("/phnix/ai/metric/iot/fe-zhangsan");
  });
});
