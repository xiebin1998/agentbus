 ...
 * - 权限档恒为 --permission-mode plan（实测只读档，禁写禁执行；沟通定位：入站恒只读）
 */
...
describe("参数装配", () => {
  it("createArgs：--session-id + -n 会话名 + -p + json 输出 + plan + prompt 收尾", () => {
    const adapter = new ClaudeAdapter({ ...cfg, sessionName: "qoder-agent" });
    const args = adapter.createArgs("请修复 bug", "sess-1");
  ...
  it("injectArgs：-r <uuid> 续接（不再带 --session-id/-n）", () => {
    ...
    const args = adapter.injectArgs("继续", "sess-1");
  ...
  it("恒只读档 → --permission-mode plan（实测只读档，禁写禁执行）", () => {
    const adapter = new ClaudeAdapter(cfg);
    const args = adapter.injectArgs("只许看不许改", "sess-1");
    expect(args[args.indexOf("--permission-mode") + 1]).toBe("plan");
  ...
  it("sessionName 缺省时不传 -n", () => {
    const adapter = new ClaudeAdapter(cfg);
    expect(adapter.createArgs("hi", "s1")).not.toContain("-n");
  });
...
describe("回合执行与输出提取", () => {
  ...
  it("inject 走 -r 续接形态，恒 plan 只读档", async () => {
    const { run, specs } = stubRun({ stdout: '{"result":"第二轮回复"}' });
    const adapter = new ClaudeAdapter(cfg, run);
    const turn = await adapter.injectWith("继续", "sess-1");
  ...
  it("非 JSON stdout 回退为裸文本（trim）", async () => {
    ...
    const adapter = new ClaudeAdapter(cfg, run);
    const turn = await adapter.injectWith("hi", "s1");
  ...
  it("JSON 无 result 字段时按 message/text/content 降级", async () => {
    ...
    const adapter = new ClaudeAdapter(cfg, run);
    expect((await adapter.injectWith("hi", "s1")).output).toBe("降级字段");
  });
