 ...
 * - 通道：publish 到 /agentbus/ai/metric/<ns>/<client_id>，payload type=metric
 */
...
async function makeMetricCollector(): Promise<{ client: MqttClient; reports: Array<{ topic: string; payload: Record<string, unknown> }> }> {
  ...
  await new Promise<void>((resolve) => client.subscribe("/agentbus/ai/metric/#", { qos: 1 }, () => resolve()));
