...
daemon 周期性 publish 指标到 /phnix/ai/metric/<ns>/<client_id>，hub 订阅
/phnix/ai/metric/# 汇总；/health 扩展返回 daemon_metrics。
...
def test_parse_metric_topic_ns_form():
    assert server.parse_metric_topic("/phnix/ai/metric/iot/fe-a") == "iot/fe-a"


def test_parse_metric_topic_flat_form():
    assert server.parse_metric_topic("/phnix/ai/metric/legacy-daemon") == "legacy-daemon"


def test_parse_metric_topic_invalid():
    assert server.parse_metric_topic("/phnix/ai/channel/x/message") is None
    assert server.parse_metric_topic("/phnix/ai/metric/") is None
    assert server.parse_metric_topic("") is None
    # 超过两段的身份非法（只取前两段以外视为非法）
    assert server.parse_metric_topic("/phnix/ai/metric/a/b/c") is None
