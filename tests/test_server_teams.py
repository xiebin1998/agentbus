...
- ACL 渲染：hub 账号全量 readwrite #；团队账号 readwrite /phnix/ai/channel/<ns>/#、
  write /phnix/ai/metric/<ns>/#（跨团队 publish 被 broker 拒，验收项）
...
def test_render_broker_acl():
    ...
    assert "topic readwrite /phnix/ai/channel/iot/#" in iot_block
    assert "topic write /phnix/ai/metric/iot/#" in iot_block
